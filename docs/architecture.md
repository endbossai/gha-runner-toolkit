...
- **Image** — `Dockerfile` builds from a pinned `ubuntu:26.04` base. Includes the `actions/runner` binary at a SHA256-pinned version, Docker CLI (talks to the mounted host socket), Node ≥ 22 (for any npm-shaped CI tooling), CPython 3.12 + 3.13 from SHA256-pinned `python-build-standalone` tarballs (placed at the `actions/setup-python` tool-cache path so the action skips its broken-on-26.04 download step), git, curl, jq, ca-certificates. No SSH server, no extra services. JDK is not baked in — `actions/setup-java` in workflows handles version selection and caches downloads inside the container.
